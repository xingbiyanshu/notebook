...
如果函数仅仅“使用”参数，不存储也不修改参数，则使用const &（基本类型除外，用值传递就好）；
如果函数不存储参数但修改参数，则使用&；
如果函数需要存储参数（即参与管理参数的生命周期），则使用智能指针，如果独占使用unique_ptr，否则使用shared_ptr（基本类型除外，用值传递就好）；
如果函数需要“接管”参数（即独占）可以使用unique_ptr，也可以使用&&，但&&涉及到移动操作，对于复杂对象这可能并不简单。一般来说参数较少用&&；
如果函数需要存储参数，如将形参存储到成员变量，且该成员变量和类的关系是组合（而非聚合），则直接使用值传递，然后使用std::move将形参移交给成员变量。
引用只能用来“使用”对象，不能通过引用参与对象的生命周期管理（比如保存该对象，如果保存也只能保存对象的副本而非引用对象本身），管理对象声明周期（独占或共享）需要使用智能指针；
